...
import 'package:fmcl/widgets/app_card.dart' show AppCard;
import 'dart:io';
import 'package:url_launcher/url_launcher.dart';
import 'package:share_plus/share_plus.dart';
import 'package:desktop_drop/desktop_drop.dart';

class ResourcepackManagementTab extends StatefulWidget {
  final String resourcepacksPath;
  const ResourcepackManagementTab({super.key, required this.resourcepacksPath});

  @override
  ResourcepackManagementTabState createState() =>
      ResourcepackManagementTabState();
}

class ResourcepackManagementTabState extends State<ResourcepackManagementTab> {
  ...
  void initState() {
    super.initState();
    _loadResourcepackFiles();
  }

  // 加载资源包文件
  Future<void> _loadResourcepackFiles() async {
    if (widget.resourcepacksPath.isEmpty) return;
    final dir = Directory(widget.resourcepacksPath);
    if (await dir.exists()) {
  ...
  // 分享资源包
  Future<void> _shareResourcePack(FileSystemEntity file) async {
    ...
    if (_isFileDisabled(file.path)) {
      try {
        ...
        LogUtil.log('准备分享资源包时出错: $e', level: 'ERROR');
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('准备分享资源包失败: $e')));
        }
        return;
      }
    }
    try {
      final params = ShareParams(
        text: '分享资源包文件: ${_getFileName(file.path)}',
  ...
  Future<void> _handleDroppedFiles(DropDoneDetails details) async {
    ...
    for (final file in files) {
      ...
      if (fileName.endsWith('.zip')) {
        try {
          ...
          final targetPath =
              '${widget.resourcepacksPath}${Platform.pathSeparator}${file.name}';
          ...
          successCount++;
          LogUtil.log('成功安装资源包: ${file.name}', level: 'INFO');
        } catch (e) {
          failCount++;
          failedFiles.add(file.name);
          LogUtil.log('安装资源包失败: ${file.name}, 错误: $e', level: 'ERROR');
        }
      } else {
        failCount++;
    ...
    if (mounted) {
      ...
      if (successCount > 0 && failCount == 0) {
        message = '成功安装 $successCount 个资源包';
      } else if (successCount > 0 && failCount > 0) {
        message = '成功安装 $successCount 个资源包，$failCount 个失败';
      } else if (failCount > 0) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton.icon(
              onPressed: () => _launchURL(widget.resourcepacksPath),
              icon: const Icon(Icons.folder_open),
              label: const Text('打开 resourcepacks 文件夹'),
            ),
          ],
        ),
      );
      ...
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text('共 ${_resourcepackFiles.length} 个资源包'),
              ElevatedButton.icon(
                onPressed: () => _launchURL(widget.resourcepacksPath),
                icon: const Icon(Icons.folder_open),
                  ...
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        ...
                        tooltip: '分享',
                        onPressed: () => _shareResourcePack(file),
                      ),
                      IconButton(
                        icon: Icon(

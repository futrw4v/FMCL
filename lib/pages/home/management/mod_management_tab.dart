...
class ModManagementTab extends StatefulWidget {
  final String modsPath;
  const ModManagementTab({super.key, required this.modsPath});

  @override
  ModManagementTabState createState() => ModManagementTabState();
}

class ModManagementTabState extends State<ModManagementTab> {
  List<FileSystemEntity> _modFiles = [];
  ...
  // 加载模组文件
  Future<void> _loadModFiles() async {
    if (widget.modsPath.isEmpty) return;
    final dir = Directory(widget.modsPath);
    if (await dir.exists()) {
      final files = dir.listSync().where((f) {
        final name = f.path.toLowerCase();
        return name.endsWith('.jar') || name.endsWith('.jar.fml');
      }).toList();
      setState(() {
        _modFiles = files;
      });
  ...
  // 分享模组
  Future<void> _shareMod(FileSystemEntity file) async {
    ...
    if (_isFileDisabled(file.path)) {
      try {
        ...
        LogUtil.log('准备分享模组时出错: $e', level: 'ERROR');
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('准备分享模组失败: $e')));
        }
        return;
      }
    }
  ...
  Future<void> _handleDroppedFiles(DropDoneDetails details) async {
    ...
    for (final file in files) {
      ...
      if (fileName.endsWith('.jar')) {
        try {
          ...
          final targetPath =
              '${widget.modsPath}${Platform.pathSeparator}${file.name}';
          ...
          successCount++;
          LogUtil.log('成功安装模组: ${file.name}', level: 'INFO');
        } catch (e) {
          failCount++;
          failedFiles.add(file.name);
          LogUtil.log('安装模组失败: ${file.name}, 错误: $e', level: 'ERROR');
        }
      } else {
        failCount++;
    ...
    if (mounted) {
      ...
      if (successCount > 0 && failCount == 0) {
        message = '成功安装 $successCount 个模组';
      } else if (successCount > 0 && failCount > 0) {
        message = '成功安装 $successCount 个模组，$failCount 个失败';
      } else if (failCount > 0) {
        message = '安装失败: ${failedFiles.join(', ')}';
      } else {
        message = '没有可安装的 .jar 文件';
      }
      ScaffoldMessenger.of(
        context,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton.icon(
              onPressed: () => _launchURL(widget.modsPath),
              icon: const Icon(Icons.folder_open),
              label: const Text('打开模组文件夹'),
            ),
          ],
        ),
      );

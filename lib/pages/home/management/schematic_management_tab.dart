...
class SchematicManagementTab extends StatefulWidget {
  final String schematicsPath;
  const SchematicManagementTab({super.key, required this.schematicsPath});

  @override
  SchematicManagementTabState createState() => SchematicManagementTabState();
}

class SchematicManagementTabState extends State<SchematicManagementTab> {
  List<FileSystemEntity> _schematicFiles = [];
  ...
  // 加载原理图文件
  Future<void> _loadSchematicFiles() async {
    if (widget.schematicsPath.isEmpty) return;
    final dir = Directory(widget.schematicsPath);
    if (await dir.exists()) {
      final files = dir.listSync().where((f) {
        final name = f.path.toLowerCase();
        return name.endsWith('.litematic') ||
            name.endsWith('.litematic.fml') ||
            name.endsWith('.schematic') ||
            name.endsWith('.schematic.fml') ||
            name.endsWith('.nbt') ||
            name.endsWith('.nbt.fml') ||
            name.endsWith('.schem') ||
            name.endsWith('.schem.fml');
      }).toList();
      setState(() {
        _schematicFiles = files;
      });
    }
  }
  ...
  // 分享原理图
  Future<void> _shareSchematic(FileSystemEntity file) async {
    ...
    if (_isFileDisabled(file.path)) {
      try {
        ...
        LogUtil.log('准备分享原理图时出错: $e', level: 'ERROR');
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('准备分享原理图失败: $e')));
        }
        return;
      }
    }
    try {
      final params = ShareParams(
        text: '分享原理图文件: ${_getFileName(file.path)}',
  ...
  Future<void> _handleDroppedFiles(DropDoneDetails details) async {
    ...
    for (final file in files) {
      ...
      if (fileName.endsWith('.litematic') ||
        ...
        try {
          ...
          final targetPath =
              '${widget.schematicsPath}${Platform.pathSeparator}${file.name}';
          ...
          successCount++;
          LogUtil.log('成功安装原理图: ${file.name}', level: 'INFO');
        } catch (e) {
          failCount++;
          failedFiles.add(file.name);
          LogUtil.log('安装原理图失败: ${file.name}, 错误: $e', level: 'ERROR');
        }
      } else {
        failCount++;
    ...
    if (mounted) {
      ...
      if (successCount > 0 && failCount == 0) {
        message = '成功安装 $successCount 个原理图';
      } else if (successCount > 0 && failCount > 0) {
        message = '成功安装 $successCount 个原理图，$failCount 个失败';
      } else if (failCount > 0) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton.icon(
              onPressed: () => _launchURL(widget.schematicsPath),
              icon: const Icon(Icons.folder_open),
              label: const Text('打开 schematics 文件夹'),
            ),
          ],
        ),
      );
      ...
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text('共 ${_schematicFiles.length} 个文件'),
              ElevatedButton.icon(
                onPressed: () => _launchURL(widget.schematicsPath),
                icon: const Icon(Icons.folder_open),
        ...
        Expanded(
          child: ListView.builder(
            itemCount: _schematicFiles.length,
            itemBuilder: (context, index) {
              final file = _schematicFiles[index];
              final fileName = _getFileName(file.path);
                  ...
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        ...
                        tooltip: '分享',
                        onPressed: () => _shareSchematic(file),
                      ),
                      IconButton(
                        icon: Icon(

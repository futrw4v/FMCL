...
class DownloadNeoForgePageState extends State<DownloadNeoForgePage> {
  ...
  Future<void> _downloadAssets() async {
    ...
    for (final hash in _assetHash) {
      ...
      if (File(fullPath).existsSync()) continue;
      final url = replaceWithMirror(
        'https://resources.download.minecraft.net/$relativePath',
      );
      downloadTasks.add({'url': url, 'path': fullPath});
    }
    if (downloadTasks.isEmpty) {
  ...
  // 提取LWJGL本地库文件的名称和路径
  Future<void> _extractLwjglNativeLibrariesPath(
    String jsonFilePath,
    String gamePath,
  ) async {
  ...
  Future<void> _executeNeoForgeInstaller() async {
    ...
    final installerPath =
        '$gamePath${Platform.pathSeparator}versions${Platform.pathSeparator}${widget.name}${Platform.pathSeparator}neoforge-installer.jar';
    final neoForgeJson =
        '$gamePath${Platform.pathSeparator}versions${Platform.pathSeparator}neoforge-${widget.neoforgeVersion}${Platform.pathSeparator}neoforge-${widget.neoforgeVersion}.json';
  ...
  Future<void> _startDownload() async {
    ...
    final neoForgeURL =
        'https://bmclapi2.bangbang93.com/maven/net/neoforged/neoforge/${widget.neoforgeVersion}/neoforge-${widget.neoforgeVersion}-installer.jar';
    try {
      ...
      try {
        ...
        setState(() {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('下载版本Json失败: $e')));
        });
        return;
      }
      // 解析游戏Json
      ...
      if (assetIndexURL != null) {
        ...
        // 提取LWJGL本地库路径
        await _extractLwjglNativeLibrariesPath(
          '$versionPath${Platform.pathSeparator}${widget.name}.json',
          gamePath,
        );
        // 提取LWJGL Natives
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          '正在下载${widget.version} + NeoForge ${widget.neoforgeVersion}',
        ),
